...
const AdminSellers: React.FC = () => {
  const [stats, setStats] = useState<SellerStats>({ totalSellers: 0, totalRevenue: 0 });
  ...
  const fetchSellerData = async () => {
    try {
      ...
      setStats({ totalSellers: res.data.totalSellers, totalRevenue: res.data.totalRevenue });
      setCurrentPage(1);
    } catch (err) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Sellers Management</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
        <Card label="Total Sellers" count={stats.totalSellers} />
        <Card label="Total Revenue" count={stats.totalRevenue} prefix="₹" />
      </div>

      <div className="overflow-x-auto bg-white rounded-xl shadow-md p-4">
        <h2 className="text-xl font-semibold mb-4 text-gray-700">Sellers List</h2>
            ...
            {paginatedData.map((seller, index) => (
              <React.Fragment key={seller._id}>
                <tr className="border-t hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4">{seller.phoneNumber}</td>
                  <td className="py-2 px-4">{seller.totalProducts}</td>
                  <td className="py-2 px-4">₹{seller.totalRevenue}</td>
                  ...
                  <td className="py-2 px-4">
                    <button onClick={() => toggleStatus(seller._id)}>
                      {seller.isActive ? (
                ...
                {expandedSeller === seller._id && seller.totalProducts > 0 && (
                  <tr className="border-t">
                    <td colSpan={9} className="py-2 px-4">
                      <div className="bg-gray-50 p-4 rounded-md">
                        <h3 className="text-sm font-semibold mb-2">Listed Products</h3>
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-gray-200 text-gray-700">
                              ...
                              <th className="py-1 px-2 text-left">Sold Date</th>
                            </tr>
                          </thead>
                          <tbody>
                            {seller.products.map((product) => (
                              <tr key={product._id} className="border-t">
                                ...
                                <td className="py-1 px-2">
                                  <span
                                    className={`px-2 py-1 rounded-full text-xs text-white ${
                                      product.isSold ? 'bg-red-500' : 'bg-green-500'
                                    }`}
                                  >
                                ...
                                <td className="py-1 px-2">
                                  {product.isSold && product.soldDate
